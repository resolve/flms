...
describe Flms::ImageLayersController do
  ...
  describe 'create' do
    describe 'access control' do
      let(:request) do
        layer = attributes_for(:image_layer).merge(start_state_keyframe_attributes: attributes_for(:start_state_keyframe),
        ...
        post :create, block_id: block_1a.id, image_layer: layer, use_route: :flms
      end
      let(:access_granted_check) { response.status == 302 && response.location == "http://test.host/flms/blocks/#{block_1a.id}/edit" }
      let(:database_performed_check) { Flms::ImageLayer.count == 1 }
      it_should_behave_like 'an action accessible only to logged-in users'
    end
    ...
    describe 'functionality' do
      it 'creates an image layer' do
        ...
        post :create, block_id: block_1a.id, image_layer: new_layer, use_route: :flms
